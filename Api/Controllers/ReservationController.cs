...
public sealed class ReservationController(IInventoryService inventory) : ControllerBase
{
    ...
    public async Task<IActionResult> SetReservation([FromBody] SetCartReservationDto dto, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(dto.CartId))
            return BadRequest(new { error = "CART_ID_REQUIRED" });

        if (dto.ProductId <= 0)
            return BadRequest(new { error = "PRODUCT_ID_INVALID" });

        if (dto.Quantity < 0)
            return BadRequest(new { error = "QTY_INVALID" });
